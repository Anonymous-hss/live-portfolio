...
// NOTE: gsap/SplitText is a Club GreenSock premium plugin. 
// If it fails to compile, we may need an alternative.
import { SplitText as GSAPSplitText } from 'gsap/dist/SplitText'; 
import { useGSAP } from '@gsap/react';
// CSS has been moved to globals.css to comply with Next.js standards

// Fallback in case SplitText is missing from the local node_modules
let GsapSplitText = GSAPSplitText;
if (!GsapSplitText) {
  try {
    GsapSplitText = require('gsap/SplitText').SplitText;
  } catch (e) {
    console.warn("GSAP SplitText is a premium plugin and was not found.");
  }
}

gsap.registerPlugin(ScrollTrigger, useGSAP);
// Only register if we actually have it
if (GsapSplitText) {
...
const Shuffle = ({
  ...
  useEffect(() => {
    const markLoaded = () => setTimeout(() => setFontsLoaded(true), 200);
    if (typeof document !== 'undefined' && 'fonts' in document) {
      if (document.fonts.status === 'loaded') markLoaded();
      else document.fonts.ready.then(markLoaded);
    } else markLoaded();
  }, []);
  ...
  useGSAP(
    () => {
      if (!ref.current || !text || !fontsLoaded || !GsapSplitText) {
        if (!GsapSplitText) {
          // If the premium plugin is missing, just render the text normally
          setReady(true);
        }
        return;
      }
      
      if (typeof window !== 'undefined' && respectReducedMotion && window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
        setReady(true);
        onShuffleComplete?.();
        return;
      }
